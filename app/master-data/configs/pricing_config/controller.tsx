...
function useTable(): TableConfig {
  ...
  return {
    columns,
    url: `${API_BASE_URL}/api/master-data/pricing/pricing-config/`,
    method: 'POST' as const,
    initSort: [{ column: 'config_id', dir: 'asc' as const }],
  };
}
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
      <div className="px-4 py-2 border-b-0 bg-muted/50">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 m-0">Pricing Config</h2>
      </div>
      <div className="flex-1 min-h-0 [&_.tabulator-header]:border-t-0">
        <TabulatorTable
          id="pricing-config-table"
          columns={columns}
          url={url}
          method={method}
          initSort={initSort}
